...
def numIslands(grid):
  ...
  def bfs(r,c):
    ...
    while queue:
      ...
      # these are the only 4 possible directions up - down - left - right
      ...
      for rd, cd in directions:
        ...
        # if the current row and column is inbounds AND we're currently at an island AND we haven't previously visited this island
        if (r in range(ROWS) and c in range(COLS) and grid[r][c] == "1" and (r,c) not in visited):
          # we append the island to the queue so we can later visit its neighbors
          queue.append((r,c))
          # we mark the current island as visited so we don't recount it
          visited.add((r,c))
