...
First, let's try the most straightforward solution.
That would probably be to spin off a new thread on each function we want to delay, sleep the request amount, and then run the function.
...
That means the number of threads we use could easily explode. We can get around this by having only one dedicated thread to call the functions, and storing the functions we need to call in some data structure.

In this case, we use a list.
We also have to do some sort of polling now to check when to run a function. We can store each function along with a unix epoch timestamp that tells it when it should run by

Then we'll poll some designated tick amount and check the list for any jobs that are due to run, run them, and then remove them from the list
'''
...
class SchedulerTwo:
  ...
  def poll(self):
    while True:
      ...
      self.fns = [(fns, due) for (fn, due) in self.fns if due > now]
      sleep(0.01)
