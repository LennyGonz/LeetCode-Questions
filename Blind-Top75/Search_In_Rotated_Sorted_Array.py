...
So brute-force : We could immediately just search through the array for the target and we can do this in O(N) time

However, if we really "think" about it - we can do this in O(log N) time using binary search

Now since the sorted input array has been rotated, MEANING some of the larger numbers have been moved to the front (we dont know the pivot point)

This creates distinct scenarios - which we'll need to think through
